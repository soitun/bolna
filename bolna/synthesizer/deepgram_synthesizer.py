...
class DeepgramSynthesizer(BaseSynthesizer):
    ...
    async def __generate_http(self, text):
        headers = {"Authorization": "Token {}".format(self.api_key), "Content-Type": "application/json"}
                ...
                if payload is not None:
                    async with session.post(url, headers=headers, json=payload) as response:
                        if response.status == 200:
                            ...
                            logger.info(
                                f"status for deepgram request {response.status} response {len(await response.read())}"
                            )
                            return chunk
                        else:
                            logger.info(
                                f"status for deepgram reques {response.status} response {await response.read()}"
                            )
                            return b"\x00"
                else:
    ...
    async def handle_interruption(self):
        ...
        try:
            if (
                self.websocket_holder["websocket"] is not None
                and self.websocket_holder["websocket"].state is websockets.protocol.State.OPEN
            ):
            ...
            try:
                ...
                if isinstance(response, bytes):
                    ...
                    if audio_chunk_count == 1 and self.ws_send_time is not None:
                        ...
                        logger.info(
                            f"Deepgram WS recv FIRST audio chunk recv_wait={recv_duration:.0f}ms time_since_send={time_since_send:.0f}ms"
                        )
                    yield response
                else:
        ...
        try:
            if self.stream:
                ...
                while True:
                    ...
                    try:
                        if (
                            "synthesizer_start_time" in meta_info
                            and "synthesizer_first_result_latency" not in meta_info
                        ):
                            meta_info["synthesizer_first_result_latency"] = (
                                time.perf_counter() - meta_info["synthesizer_start_time"]
                            )
                    ...
                    try:
                        if "synthesizer_start_time" in meta_info:
                            meta_info["synthesizer_total_stream_duration"] = (
                                time.perf_counter() - meta_info["synthesizer_start_time"]
                            )
                    except Exception:
                        pass
            ...
            if (
                ...
                if result is None:
                    ...
                    if consecutive_failures >= max_failures:
                        logger.error(
                            "Max connection failures reached for Deepgram TTS - stopping reconnection attempts"
                        )
