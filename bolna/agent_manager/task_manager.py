...
class TaskManager(BaseManager):
    ...
    async def __do_llm_generation(
            ...
            async for llm_message in self.tools["llm_agent"].generate(
                ...
                if self.stream:
                    ...
                    filler_message = compute_function_pre_call_message(
                        detected_lang, function_tool, function_tool_message
                    )
        ...
        if self.stream and llm_response != filler_message:
            self.__store_into_history(
                meta_info, messages, llm_response, should_trigger_function_call=should_trigger_function_call
            )
    ...
    async def _handle_transcriber_output(self, next_task, transcriber_message, meta_info):
        ...
        convert_to_request_log(
            ...
            meta_info=meta_info,
            model=self.task_config["tools_config"]["transcriber"]["provider"],
            run_id=self.run_id,
        )
        if next_task == "llm":
            ...
            if (
                ...
                if self.check_if_user_online:
                    ...
                    user_online_message = select_message_by_language(
                        self.check_user_online_message_config, detected_lang
                    )
    ...
    async def __start_transmitting_ambient_noise(self):
        try:
            audio = await get_raw_audio_bytes(
                f"{os.getenv('AMBIENT_NOISE_PRESETS_DIR')}/{self.soundtrack}", local=True, is_location=True
            )
